...
class PPO_MORL(ActorCriticRLModel):
    """
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4, vf_coef=0.5,
        ...
        super(PPO_MORL, self).__init__(policy=policy, env=env, verbose=verbose, requires_vec_env=True,
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    with tf.variable_scope('model'):
                        ...
                        for param in self.params:
                            ...
                            self.params_all.append(layer_params)
                        self.restores_all = []
                        for p, loaded_p in zip(self.params, self.params_all):
                            self.restores_all.append(p.assign(loaded_p))
    ...
    def collect_batch_and_update(self, runner, weight, batch_size, lr_now, cliprangenow):
        obs, returns, masks, actions, values, neglogpacs, states, ep_infos, true_reward = runner.run(weight)
